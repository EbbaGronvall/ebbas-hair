...
class Booking(models.Model):
    ...
    stylist = models.ForeignKey(Stylist, on_delete=models.CASCADE,
                                related_name="stylist_booking", null=True,
                                blank=True)
    ...
    def clean(self):
        ...
        booking_datetime = datetime.combine(self.day,
                                            datetime.strptime(self.time,
                                                              "%H:%M").time())
        ...
        if Booking.objects.filter(day=self.day, time=self.time,
                                  stylist=self.stylist).exists():
            raise ValidationError('The selected time slot is already booked' +
                                  ' for this stylist.')

    def __str__(self):
        return f'''Booking by {self.customer.username} with
         {self.stylist.stylist_name.username} on {self.day} at {self.time}'''
