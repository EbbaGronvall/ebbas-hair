    ...
    if request.method == 'POST':
        if 'update_booking' in request.POST:
            ...
            if new_day_date.weekday() >= 5:  # 5 is Saturday, 6 is Sunday
                messages.error(request, 'You cannot book an appointment on a weekend.')
            elif Booking.objects.filter(day=new_day, time=new_time, stylist=booking.stylist).exists():
